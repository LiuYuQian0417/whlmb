...
/**
 * 注意
 * 配置文件rsa.php位于 application/extra目录下
...
/**
 * 对接app
 * 需要提供给APP端rsa配置文件里的 request_pubKey 和 response_privateKey
 */
...
/**
 * 如何设置明文请求调试
 * 1.在rsa_path文件中将debug设置为true
 */
class RSACrypt
{
    //将配置文件自动放置在 tp 额外配置目录下
    const rsa_path = 'config' . DIRECTORY_SEPARATOR . 'rsa.php';
    ...
    static function makeRSAKey()
    {
        ...
        if (!file_exists(self::$rootPath . self::path))
            mkdir(self::$rootPath . self::path);
        //检查到不存在 rsa 配置文件,则创建
        $path_name = self::$rootPath . self::rsa_path;
        if (!file_exists($path_name)) {
            $config = array('private_key_bits' => 1024);
            $res = openssl_pkey_new($config);
            openssl_pkey_export($res, $request_privateKey);
            $request_pubKey = openssl_pkey_get_details($res);
            $request_pubKey = $request_pubKey["key"];

            $res = openssl_pkey_new($config);
            openssl_pkey_export($res, $response_privateKey);
...
return [
    'debug'=>true,
    'request_privateKey'=>'{$request_privateKey}',
    'request_pubKey'=>'{$request_pubKey}',
    'response_privateKey'=>'{$response_privateKey}',
...
EOT;
    ...
    {
        self::$rootPath = dirname(Env::get('app_path')) . DIRECTORY_SEPARATOR;
        if (!file_exists(self::$rootPath . self::rsa_path))
    ...
    public function request($rule = '')
    {
        ...
        if (config('rsa.debug')) {
            ...
            $request_param = self::request_decrypt($param, config('rsa.request_privateKey'));
            error_log(var_export($request_param, true), 3, './logger.log');
        }
        //验证请求参数
        ...
        //如果严格设定了请求参数规则,则进行检验
        if ($rule) $this->paramRuleCheck($request_param, $rule);
    ...
    /**
     * 单独解密
     * @param $args string 需要解密的字符串
     * @return null
     */
    public function singleDec($args = '')
    {
        if ($args === '') return null;
        $decryption = self::request_decrypt($args, config('rsa.request_privateKey'));
     ...
     * @return null
     */
    public function singleEnc($args = [])
    {
        if (empty($args)) return null;
        $encryption = self::response_encrypt($args, config('rsa.request_pubKey'));
     ...
     * @return mixed
     */
    public function responseEnc($response_arr, $encrypt = false)
    {
        //如果开启了 debug 模式,所有返回都强制为明文
//        if (config('rsa.debug'))
//            $encrypt = false;
        $response_data = $encrypt ?
            self::response_encrypt($response_arr, config('rsa.response_pubKey')) :
            $response_arr;
     ...
     * @return mixed
     */
    public function response($response_arr, $encrypt = false)
    {
        //如果开启了 debug 模式,所有返回都强制为明文
        if (config('rsa.debug'))
            $encrypt = false;
    ...
    /**
     * 使用$request_privateKey为请求密文解密
     * 先对请求密文进行base64解码,再进行密文解密
     * @param $param ,请求密文
     * @param $request_privateKey ,私钥,RSA算法,为请求密文数据解密用,app端存有对应公钥
     * @return mixed
     */
    protected function request_decrypt($param, $request_privateKey)
    {
        $decrypted = $this->ssl_decrypt(base64_decode($param), 'private', $request_privateKey);
