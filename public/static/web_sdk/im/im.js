...
;(function (RCS) {
    ...
    var send_get = function (a) {
		...
		xhr.open('post',url + '/client/store/rongInfo',false);
    ...
    var pushMessage = function (msg) {
        if (terminal == 'pc') {
            ...
            var options = {
                body: '您有一条新消息，请及时回复',
                icon: (msg.content.user && msg.content.user.icon) ? msg.content.user.icon : '/client/static/im/images/kefu.png',
            }
    ...
    var loadHisMessages = function () {
        var callbacks = function (list, hasMsg) {
            ...
            messageList.list = modificateMessage(send_get(conversation.messageContent))
            for (let i = 0; i < messageList.list.length ; i++){
                messageList.list[i].name =  messageList.list[i].nickname
            }
    ...
    var startConversation = function (event) {
        ...
        conversation.name = event.currentTarget.getAttribute('_cname')
        conversation.avatar = event.currentTarget.getAttribute('_avatar')
    ...
    var openConversation = function (conversation) {
        ...
        var callbacks = function (list, hasMsg) {
            ...
            data.targetName = '用户：' + conversation.name
            data.terminal = terminal
			data.nickname = conversation.name
        ...
        getHisMessage(conversation.id, 0, parseInt(count), callbacks, conversation.name, conversation.avatar)
    }

    //拉去消息记录
    var getHisMessage = function (conversationId, timestrap, count, callbacks, name, avatar) {
        var conversationType = RongIMLib.ConversationType.PRIVATE //私聊,其他会话选择相应的消息类型即可。1

		var targetId = conversationId // 想获取自己和谁的历史消息，targetId 赋值为对方的 Id。bb
        // timestrap默认传 null，若从头开始获取历史消息，请赋值为 0 ,timestrap = 0;
        // count每次获取的历史消息条数，范围 0-20 条，可以多次获取。2
        RongIMLib.RongIMClient.getInstance().getHistoryMessages(conversationType, targetId, timestrap, count, {
            ...
            onError: function (error) {
                console.log('GetHistoryMessages,error_code:' + error)
            }
        })
    }
    ...
    var sdkInit = function (params, callbacks) {
        ...
        RongIMClient.setOnReceiveMessageListener({
            ...
            onReceived: function (message) {
                var src = '/static/web_sdk/video/6005.mp3'
                playSound(src)
                // 判断消息类型
