/*
 ...
 num：匹配数值型
 qq：匹配qq号码
 unequal：当前值不能等于被检测的值，如可以用来检测新密码不能与旧密码一栿
 notvalued：当前值不能包含指定值，如密码不能包含用户名等的检浿
 min：多选框最少选择多少顿
 max：多选框最多不能超过多少项
 byterange:判断字符长度，中文算两个字符
 numrange：判断数值范围，如小亿100大于10之间的数
 daterange：判断日期范囿
 idcard：对身份证号码进行严格验诿
 */
    ...
    if($.Datatype){
        ...
        $.extend($.Datatype,{
            ...
            "unequal":function(gets,obj,curform,datatype){
                /*
                 当前值不能与指定表单元素的值一样，如新密码不能与旧密码一样，密码不能设置为用户名筿
                 注意需要通过绑定with属性来指定要比较的表单元素，可以是clas，id或者是name属性倿
                 ...
                 也可以用来验证不能与with指定的值相筿
                 当上面根据class，id和name都查找不到对象时，会直接跟with的值比辿
            ...
            "notvalued":function(gets,obj,curform,datatype){
                /*
                 当前文本框的值不能含有指定文本框的值，如注册时设置的密码里不能包含用户吿
                 注意需要给表单元素绑定with属性来指定要比较的表单元素，可以是clas，id或者是name属性倿
            ...
            "min":function(gets,obj,curform,datatype){
                /*
                 checkbox最少选择n顿
                 注意需要给表单元素绑定min属性来指定是至少需要选择几项，没有绑定的话使用默认倿
            ...
            "max":function(gets,obj,curform,datatype){
                /*
                 checkbox最多选择n顿
                 注意需要给表单元素绑定max属性来指定是最多需要选择几项，没有绑定的话使用默认倿
            ...
            "byterange":function(gets,obj,curform,datatype){
                /*
                 判断字符长度，中文算两个字符
                 注意需要给表单元素绑定max,min属性来指定最大或最小允许的字符长度，没有绑定的话使用默认倿
                 */
                var dregx=/[^\x00-\xff]/g;
            ...
            "numrange":function(gets,obj,curform,datatype){
                /*
                 判断数值范囿
                 注意需要给表单元素绑定max,min属性来指定最大或最小可输入的值，没有绑定的话使用默认倿
                 */
                ...
                if(gets<minim){
                    return "值不能小亿"+minim+"＿";
                }else if(gets>maxim){
                    return "值不能大亿"+maxim+"＿";
                }
                return  true;
            },
            ...
            "daterange":function(gets,obj,curform,datatype){
                /*
                 判断日期范围
                 注意需要给表单元素绑定max或min属性，或两个同时绑定来指定最大或最小可输入的日朿
                 日期格式＿2012/12/29 房 2012-12-29 房 2012.12.29 房 2012,12,29
                 */
            ...
            "idcard":function(gets,obj,curform,datatype){
                ...
                if (gets.length == 15) {
                    ...
                    var a_idCard = gets.split("");// 得到身份证数绿
                ...
                function isTrueValidateCodeBy18IdCard(a_idCard) {
                    ...
                    if (a_idCard[17].toLowerCase() == 'x') {
                        a_idCard[17] = 10;// 将最后位为x的验证码替换丿10方便后续操作
                    }
                    for ( var i = 0; i < 17; i++) {
