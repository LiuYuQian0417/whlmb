(function (window, main, attrControl) {
    ...
    $(document).on('click', '.brand_form .brand-container .brand-list li', function () {
        ...
        $('[name="brand_name"]').val(this.title).css({background: 'url(/template/master/resource/image/common/xljt.png) 270px 16px no-repeat rgb(255, 255, 255)'});
        ...
        $('.brand_name_keyword').val(this.title).css({background: 'url(/template/master/resource/image/common/xljt.png) 270px 16px no-repeat rgb(255, 255, 255)'});
        ...
        $('.drop-down').css('backgroud', '#fff url(/template/master/resource/image/common/xljt.png) 200px 16px no-repeat');
        $('.cate-container').hide();
        $('.drop-down-cate').css('background', '#fff url(/template/master/resource/image/common/xljt.png) 410px 16px no-repeat');
    ...
    $('.drop-down').on('click', function (e) {
        ...
        var style = ($(this).siblings('.brand-container').toggle().css('display')) == 'none' ?
            '#fff url(/template/master/resource/image/common/xljt.png) 270px 16px no-repeat' :
            '#fff url(/template/master/resource/image/common/xljts.png) 270px 16px no-repeat';
    ...
    $('.drop-down-cate').on('click', function (event) {
        ...
        var style = ($('.cate-container').toggle().css('display')) == 'none' ?
            '#fff url(/template/master/resource/image/common/xljt.png) 410px 16px no-repeat' :
            '#fff url(/template/master/resource/image/common/xljts.png) 410px 16px no-repeat';
    ...
    window.step = {
                    ...
                    if (len === ind) {
                        ...
                        extra.ajax({
                            data: formSer, url: u, callback: function (res) {
                                ...
                                if (res.code === 0) {
                                    window.location.href = '/client/goods/index'
                                }
                            }
                        })
                    } else {
        ...
        getSon: function (type, id) {
            ...
            if (type < 3) {
                ...
                main.ajax({
                    data: {'id': id, 'type': type}, url: '/client/goods/getSonCate', callback: function (res) {
                        if (res.code === 0) {
        ...
        getCate: function (cateId, count, keyword) {
            ...
            main.ajax({
                data: {'id': cateId || '', 'keyword': keyword || ''},
                url: '/client/goods/getCate',
                callback: function (res) {
                    if (res.code == 0) {
        ...
        batchGoods: function () {
            ...
            window.location.href = '/client/goods_batch_operation/index?cateId=' + cateId + '&brandId=' + brandId + '&keyword=' + keyword + '&search=' + 1;
        },
        exportGoods: function () {
            var cateId = $('.cate_id_keyword').val(),
                brandId = $('.brand_id_keyword').val(),
                keyword = $('.search_keyword').val();
            window.location.href = '/client/goods_export/index?cateId=' + cateId + '&brandId=' + brandId + '&keyword=' + keyword + '&search=' + 1;
        },
        ...
        // 打开frame
        openCity: function (obj, fid) {
            var param = 'freightId=' + $('#freight_id').val() + '&fid=' + fid;
            layer.open({
                type: 2,
                title: obj.title,
                move: false,
                ...
                area: ['100%', '700px'],
                btn: obj.btn,
                content: '/client/goods/city?' + param
            });
        },
        ...
        addFont: function (c, maxLength) {
            layer.prompt({
                ...
                var html = '<div class="m-txt" style="font-size: 14px;">' + text + step.toolsHtml + '</div>';
                $(c).append(html);
                main.tools.sort($('div[class^="m-"]'));
                layer.close(index);
            });
