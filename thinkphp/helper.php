...
if (!function_exists('request')) {
     ...
     */
    function request($type = true)
    {
        $req = app('request');
        // 需要检测是否含有mid
        if (!$type && !$req->mid) {
            \think\facade\Response::create(['code' => -201, 'message' => '登录过期,请重新登录'], 'json')->send();
        }
        if (isset($req->mid) && $req->mid) {
            //判断会员是否已经被删除
            $member_info = (new \app\common\model\Member())
                ->where([
                    ['member_id', '=', $req->mid],
                ])
                ->field('status,pay_points,phone,web_open_id,subscribe_time,nickname,micro_open_id')
                ->find();
            if (is_null($member_info) || !$member_info['status']) {
                \think\facade\Response::create(['code' => -201, 'message' => '该账号已被注销或禁用'], 'json')->send();
            } else {
                $req->pay_points = $member_info['pay_points'];
                $req->web_open_id = $member_info['web_open_id'];
                $req->micro_open_id = $member_info['micro_open_id'];
                $req->subscribe_time = $member_info['subscribe_time'];
                $req->nickname = $member_info['nickname'];
                $req->phone = $member_info['phone'];
            }
        }
