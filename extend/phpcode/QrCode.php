...
class QrCode
{
     ...
     * 获取条形码
     */
    public static function getBarCode($number, $filePath = false, $size = 2, $height = 30, $showfront = false)
    {
        // Barcode Part
     ...
     * 获取二维码图片
     */
    public static function getQrCode($data, $parameter, $filePath = false, $size = '10', $level = 'L')
    {
        ...
        if ($filePath) {
            ...
            if (isset($data)) {
                //it's very important!
                if (trim($data) == '') throw new BadRequestHttpException('data 不能为空！');
                // user data
                $filename = $filePath . $data . '.png';
                \QRcode::png($parameter, $filename, $errorCorrectionLevel, $matrixPointSize, 2);
                return $filename;
            }
        } else {
            if (isset($data)) {
                ...
                if (trim($data) == '') throw new BadRequestHttpException('data 不能为空！');
                ob_clean();//清空（擦掉）输出缓冲区
                \QRcode::png($parameter, false, $errorCorrectionLevel, $matrixPointSize, 2);
                exit();
