...
class Order extends Controller
{
     ...
     * @return array|mixed
     */
    public function general(OrderAttachModel $orderAttach, OrderGoods $orderGoods)
    {

        try {
            $_where = [];
            $_field = 'IFNULL(sum(case when distribution_type = 3 then 1 else 0 end),0) as express_mail,
                IFNULL(sum(case when distribution_type = 1 then 1 else 0 end),0) as city_express,
                IFNULL(sum(case when distribution_type = 2 then 1 else 0 end),0) as self_help,
                IFNULL(sum(case when pay_type = 2 then 1 else 0 end),0) as pay_on_delivery,
                IFNULL(sum(case when status = 0 and distribution_type in (1,2,3) then 1 else 0 end),0) as non_payment,
                IFNULL(sum(case when status = 1 and distribution_type in (1,2,3) then 1 else 0 end),0) as non_deliver,
                IFNULL(sum(case when status = 2 and distribution_type in (1,2,3) then 1 else 0 end),0) as deliver,
                IFNULL(sum(case when status = 3 and distribution_type in (1,2,3) then 1 else 0 end),0) as done,
                IFNULL(sum(case when status = 4 and distribution_type in (1,2,3) then 1 else 0 end),0) as closed';
            ...
            if (\config('user.one_more') != 1) {
                ...
                $_where = [
                    ['store_id', '=', $_oneStoreId],
                ];
            }


            // 获取数据
            // express_mail 快递邮寄，city_express 同城速递，self_help 门店自提
            // non_payment 待支付，non_deliver 待发货，deliver 已发货，done 已完成，closed 已关闭，refund 售后
            $data = $orderAttach::withTrashed()
                ->where($_where)
                ->field($_field)
                ->cache(TRUE, 60)
                ->find();
            $data['refund'] = $orderGoods->where([
                ['status', 'in', '5.1,5.2,5.3,5.4'],
            ])->cache(TRUE, 60)->count('order_goods_id');
            ...
            $orderPayTotal = [];
            foreach ($date as $k => $v) {
                $orderCount = $orderAttach::withTrashed()
                    ->whereBetweenTime('create_time', $v)
                    ->where($_where)
                    ->cache(TRUE, 60, 'orderCount')
                    ->count('order_attach_id');

                array_push($orderTotal, $orderCount);

                $orderPayCount = $orderAttach::withTrashed()
                    ->whereBetweenTime('create_time', $v)
                    ->where('status', '<>', '0')
                    ->where('status', '<>', '6')
                    ->where($_where)
                    ->cache(TRUE, 60, 'orderPayCount')
                    ->count('order_attach_id');
                array_push($orderPay, $orderPayCount);

                $payTotal = $orderAttach::withTrashed()
                    ->whereBetweenTime('create_time', $v)
                    ->where('status', '<>', '0')
                    ->where('status', '<>', '6')
                    ->where($_where)
                    ->cache(TRUE, 60, 'payTotal')
                    ->sum('subtotal_price');
                array_push($orderPayTotal, $payTotal);
            }
        ...
        return $this->fetch('', [
            ...
            'order_total' => json_encode($orderTotal),
            'order_pay' => json_encode($orderPay),
            'order_pay_total' => json_encode($orderPayTotal),
        ]);
    }
     ...
     * @return array|mixed
     */
    public function settings(Request $request, Config $config)
    {
        ...
        if ($request::isPost()) {
            ...
            try {
                $param = $request::post();
                $config->valid($param, 'order_settings');
                ...
                Db::commit();
                return ['code' => 0, 'message' => config('message.')[0], 'url' => '/order/settings'];
    ...
    public function index(Request $request,
                          OrderAttachModel $orderAttach,
                          OrderGoodsModel $orderGoods,
                          CheckOrderModel $checkOrder,
                          StoreModel $store)
    {
        try {
            // 获取数据
            $param = $request::get();
            // 筛选条件
            $condition[] = ['store.shop', 'in', '0,1,2'];
            $field = 'order_attach.create_time';
            if (array_key_exists('origin_type', $param) && reset($param['origin_type']) != 0) {
                $condition[] = ['origin_type', 'in', implode(',', $param['origin_type'])]; // 订单来源
            } else {
                $condition[] = ['origin_type', 'in', '1,2,3,4']; // 订单来源
            }

            if (array_key_exists('order_type', $param) && reset($param['order_type']) != 0) {
                $condition[] = ['order_type', 'in', implode(',', $param['order_type'])];// 订单类型
            } else {
                $condition[] = ['order_type', 'in', '1,2,3,4'];
            }
            if (array_key_exists('distribution_type', $param) && reset($param['distribution_type']) != 0) {
            ...
            $whereOr = [];
            if (array_key_exists('status', $param) && reset($param['status']) != -1) {
                // 存在退款中订单
                if (in_array(5, $param['status'])) {
                    ...
                    if (count($param['status']) == 1) {
                        ...
                        if ($orderGoodsCol) array_push($whereOr, ['order_attach.order_attach_id', 'in', implode(',', array_unique($orderGoodsCol))]);
            ...
            if (array_key_exists('pay_channel', $param) && reset($param['pay_channel']) != -1) {
            ...
            if (array_key_exists('type', $param) && $param['type'] != 0) $condition[] = ['distribution_type', 'eq', $param['type']]; // 支付方式

            // 单店
            if ($_oneMore != 1) {
                $condition[] = ['order_attach.store_id', 'eq', $_oneStoreId];
            } else {
                if (array_key_exists('shop', $param) && $param['shop'] != -1) $condition[] = ['store.shop', 'eq', $param['shop']];                       // 店铺类型
                if (array_key_exists('store_id', $param) && $param['store_id'] != 0) $condition[] = ['order_attach.store_id', 'eq', $param['store_id']]; // 自营店铺
            }
            if (array_key_exists('pay_type', $param)) $condition[] = ['pay_type', 'eq', $param['pay_type']]; // 支付类型 1线上 2货到付款
            ...
            if ($param['start_date'] ?? '') {
                $condition[] = [$field, '>=', $param['start_date']];
            }
            if ($param['end_date'] ?? '') {
                $condition[] = [$field, '<=', $param['end_date']];
            }
            $conditionSql = [];
            // 关键词
            if (array_key_exists('keywords_type', $param) && $param['keywords_type'] != 0 && $param['keywords'] != '' && $param['keywords_type'] != 1 && $param['keywords_type'] != 2) {
            ...
            } else if ($param['keywords_type'] == 1 && $param['keywords'] != '') {
                //  1.商品名称
                $orderGoodsCol = $orderGoods->where([
            ...
            } else if ($param['keywords_type'] == 2 && $param['keywords'] != '') {
                //  2 .商品货号
                $orderGoodsCol = $orderGoods->where([
                ...
                if ($param['keywords']) {
                    ...
                    $conditionSql[] = ["order_attach.order_attach_id", 'in', implode(',', array_unique($orderGoodsCol))];
                    $condition[] = ['nickname|consignee_name|consignee_phone|order_attach_number', 'like', '%' . $param['keywords'] . '%'];
                }
            }
            if ($param['export_flag'] ?? '') {
                $e['data'] = $orderGoods
                    ->alias('og')
                    ->join('order_attach order_attach', 'order_attach.order_attach_id = og.order_attach_id')
                    ...
                    ->join('group_activity group_activity', 'group_activity.group_activity_id = order_attach.group_activity_id', 'left')
                    ->where($condition)
                    ->whereOr($conditionSql)
                    ->field('og.goods_name,og.quantity,og.single_price,og.subtotal_price,order_attach.*,member.nickname,
                    store.shop,store.store_name,order.order_type,group_activity.status as group_activity_status,address_province,address_city,
                    address_area,address_street,address_details,consignee_name,consignee_phone,order.origin_type,og.status as goods_status')
                    ->whereOr($whereOr)
                    ->order(['order.create_time' => 'desc'])
                    ->select();
                if (!$e['data']->isEmpty()) {
                    $e['allowField'] = [
                        'order_attach_number' => '订单编号',
                        'goods_name' => '商品名称',
                        'single_price' => '商品单价',
                        'quantity' => '商品数量',
                        'create_time' => '下单时间',
                    ];
                    foreach ($param as $k => $q) {
                        if (stripos($k, 'exp_') === 0) {
                            $e['allowField'][substr($k, 4)] = $q;
                        }
                    }
                    foreach ($e['data'] as $val) {
                        // 订单类型
                        if ($e['allowField']['order_type'] ?? '') {
                            $val->order_type = [
                                1 => '普通',
                                2 => is_null($val->group_activity_status) ? '拼团' : [1 => '拼团（进行中）', 2 => '拼团（成功）', 3 => '拼团（失败）'][$val->group_activity_status],
                                3 => '砍价',
                                4 => '限时抢购',
                                5 => '线下',
                            ][$val->order_type];
                        ...
                        // 支付方式
                        if ($e['allowField']['pay_channel'] ?? '') {
                            $val->pay_channel = ['待付款', '微信', '支付宝', '余额', '银行卡', '线下', '货到付款',][$val->pay_channel];
                        }
                        // 订单状态
                        if ($e['allowField']['status'] ?? '') {
                            $extra_status_text = '';
                            if (in_array($val->goods_status, ['4.2', '4.3', '5.1', '5.2', '5.3', '5.4', '5.5', '5.6', '5.7'])) {
                                $extra_status_text = [
                                    '4.2' => '（退款成功）',
                                    '4.3' => '（退货成功）',
                                    '5.1' => '（申请退款中）',
                            ...
                            $val->status = [
                                '待付款',
                                $val->distribution_type == 1 ? '待配送' : '待发货',
                                [1 => '配送中', 2 => '待自提', 3 => '待收货', 4 => '待收货'][$val->distribution_type],
                                '已完成',
                                "已关闭{$extra_status_text}",
                                "退款中{$extra_status_text}",
                                '已取消',
                            ][$val->status];
                        }
                        if ($e['allowField']['address'] ?? '') {
                            $val->address = $val->address_province . $val->address_city . $val->address_area .
                                $val->address_street . $val->address_details;
                        }
                    }
                    Hook::exec(["app\\common\\service\\Export", "out"], $e);
                }
                exit();
            } else {
                // 获取数据
                $data = $orderAttach::withTrashed()
                    ...
                    ->where($condition)
                    ->whereOr($conditionSql)
                    ->field('order_attach.*,member.nickname,order_attach.pay_channel,order_attach.status,order_attach.order_attach_number,store.shop,
                store.store_name,order.order_type,group_activity.status as group_activity_status,address_province,address_city,address_area,address_street,
                ...
                foreach ($data as $order_attach) {
                    $orderGoodsArr = $orderGoods::withTrashed()
                        ->where(['order_attach_id' => $order_attach['order_attach_id']])
                        ->select()
                        ->toArray();
    ...
    public function get_examine(Request $request, OrderGoodsModel $orderGoods, OrderAttachModel $orderAttach)
    {
        try {
            ...
            $item = $orderAttach::with(['orders' => function ($query) {
                ...
                ->field('store_id,member_id,order_id,create_time,subtotal_price,subtotal_share_platform_coupon_price,subtotal_share_platform_packet_price,subtotal_coupon_price,subtotal_promotion_price,subtotal_fullSub_price')
                ->find();
            $goods = $orderGoods
                ->where(['order_attach_id' => $order_attach_id])
            ...
            if (config('user.one_more') == 0) {
                $item = $item->toArray();
                Env::load(Env::get('app_path') . 'common/ini/.config');
                $item['store']['store_name'] = Env::get('title');
            } else {
                $item->store;
            }
            $item['orders']['consignee_name'] = mb_substr($item['orders']['consignee_name'], 0, 1) . ($item['member']['sex'] == 0 ? '女士' : '先生');
     ...
     * @return mixed
     */
    public function examine(Request $request, OrderAttachModel $orderAttach, ExpressModel $express, StoreModel $store, OrderGoodsModel $orderGoods, MessageModel $message, DadaShop $dadaShop, DadaMerchant $dadaMerchant, Invoice $invoice)
    {
        if ($request::isPost()) {
            Db::startTrans();
            ...
            try {
                ...
                $attach = $orderAttach
                    ...
                    ->field('order_attach_id,order_number,dada,order_attach_number,distribution_tel,
                    prepay_id,express_value,express_number,client_id')
                    ->find();
                $goods = $orderGoods->where(['order_attach_id' => $param['order_attach_id']])->field('file,goods_name,member_id')->find();
                ...
                } else if ($param['distribution_type'] == 1) {
                    // 同城速递
                    if ($param['delivery_method'] == 1) {
                        // 快递配送
                        $check = $orderAttach->valid($param, 'examine');
                    } else if ($param['delivery_method'] == 2) {
                        // 自主配送
                ...
                foreach (Db::name('order_goods')->where('order_goods_id', 'in', $orderGoodsStr)->column('status') as $k => $v) {
                ...
                if ($result['code'] == 0) {
                    // 计划任务自动收货
                    Env::load(Env::get('app_path') . 'common/ini/.config');
                    $collectTime = Env::get('confirm_receipt') * 86400 ?: 5 * 86400;
                ...
                return ['code' => 0, 'message' => config('message.')[0], 'url' => 'order/index'];
        ...
        try {
            ...
            $goods = $orderGoods
                ...
                ->where(['order_goods.order_attach_id' => $request::get('id')])
                ->alias('order_goods')
            ...
            $thirdCityInfo = $thirdInfo = $dadaExpress = [];
            if ($merchantInfo) {
                // 查询商户城市信息
                $dadaRet = (new Dada($merchantInfo['source_id'], []))->request('/api/cityCode/list');
//            if ($dadaRet['code']) {
//                throw new \Exception('请求达达城市信息失败');
//            }
                $thirdCityInfo = $dadaRet['data'];
                $thirdInfo = $dadaShop
                    ->where([
                        ['store_id', '=', $merchantInfo['store_id']],
                        ['status', '=', 1],
                    ])
                    ->field('delete_time', TRUE)
                    ->select();
                ...
                // 达达配送
                if ($item['dada'] == 1) {
                    $dadaExpress = (new Dada($merchantInfo['source_id'], ['order_id' => $item['order_attach_id']]))->request('api/order/status/query');
                }
            }
            ...
            $data['param'] = json_encode(['com' => $item['express_value'], 'num' => trim($item['express_number'])]);
            ...
            $store = $store->where(['store_id' => Session::get('client_store_id')])->field('province,city,area,address')->find();
    ...
    public function refunds_details(Request $request, OrderGoodsModel $goods, Invoice $invoice)
    {
        ...
        $item = $goods
            ...
            ->where('order_goods_id', $request::get('id'))
            ->field('order_goods.order_attach_id,order_goods.order_goods_id,order_goods.member_id,order_goods.attr,order_goods.quantity,order_goods.single_price,
            ((single_price*quantity)-order_goods.sub_share_shop_coupon_price-order_goods.sub_share_platform_coupon_price-order_goods.subtotal_share_platform_packet_price) as goodsTotal,
            ((single_price*quantity)+order_goods.sub_freight_price-order_goods.sub_share_shop_coupon_price-order_goods.sub_share_platform_coupon_price-order_goods.subtotal_share_platform_packet_price) as goodsTotal1,
            order_goods.sub_freight_price,order_goods.goods_name,order_goods.file,order_goods.status,member.nickname,order_goods.redo_status,order_attach.distribution_type')
            ->find();
    ...
    public function checkRefunds(Request $request, OrderGoodsRefundModel $orderGoodsRefund)
    {
        if ($request::isPost()) {
            ...
            try {
                ...
                if (empty($orderGoodsRefund->where(['order_goods_id' => $param['id']])->find())) return ['code' => -100, 'message' => '订单状态错误，请刷新页面'];
     ...
     * @return array|mixed
     */
    public function editPrice(Request $request, AlterPriceLog $alterPriceLog, OrderAttachModel $orderAttach)
    {
        if ($request::isPost()) {
...
//                $orderAttach->isUpdate(true)->allowField(true)->save($param);
        ...
        return $this->fetch('', [
            //'item' => $orderAttach->where(['order_attach_id' => $request::get('order_attach_id')])->field('order_attach_id,subtotal_freight_price,subtotal_price')->find(),
        ]);
    }
        ...
        if ($request::isPost()) {
            ...
            try {
                ...
                $find = $orderAttach
                    ...
                    ->field('oa.order_attach_id,oa.take_code,oa.after_sale_times,oa.member_id,
                    s.store_name,m.web_open_id,m.nickname,m.subscribe_time,m.micro_open_id,m.phone')
                    ->find();
                ...
                $status = Db::name('order_goods')
                    ...
                    ->field('order_goods_id,status,file')
                    ->select();
                $orderGoodsId = [];
                $file = '';
                ...
                $pushServer->send([
                    'tplKey' => 'order_state',
                    'openId' => $find['web_open_id'],
                    'subscribe_time' => $find['subscribe_time'],
                    'microId' => $find['micro_open_id'],
                    'phone' => $find['phone'],
                    'data' => [7, $find['store_name'], $find['nickname']],
                    'inside_data' => [
                        'member_id' => $find['member_id'],
                        'type' => 1,
                        'jump_state' => '0',
                        'attach_id' => $find['order_attach_id'],
                        'file' => $file,
                    ],
                    'sms_data' => [],
                ], 2);
    ...
    public function closeOrder(Request $request, OrderAttach $orderAttach, OrderGoods $orderGoods, Goods $goods, Products $products, Lock $lock, MemberCoupon $memberCoupon, Limit $limit)
    {
        try {
            ...
            $orderAttachData = $orderAttach
                ...
                ->find();
            if (!$orderAttachData) return ['code' => -1, 'message' => config('message.')[-11][-3]];
            ...
            if (!$lockData) return ['code' => -4, 'message' => config('message.')[-11][-4]];
    ...
    public function offline_payment_list(OrderAttachModel $orderAttach, CheckOrder $checkOrder)
    {
        ...
        if ($this->request->isPost()) {
            ...
            $orderAttach = $orderAttach->whereTime('create_time', [!empty($param['start_date']) ? $param['start_date'] : '1968-01-01 00:00:00', !empty($param['end_date']) ? $param['end_date'] : date('Y-m-d H:i:s', time())]);
        }
        $data = $orderAttach->with(['member' => function ($query) {
            $query->field('nickname,member_id');
        }, 'orderGoods' => function ($query) {
            $query->field('original_price,order_attach_id');
        }])->where($where)
            ->field('order_attach_number,create_time,subtotal_price,subtotal_coupon_price,subtotal_share_platform_packet_price,order_attach_id,member_id,store_id')
            ->order('pay_time', 'desc');
        ...
        if ($this->request->has('dc')) {
            ...
            $data = $data->select();
            foreach ($data as $vv) {
                $dataArr[] = [$vv['order_attach_number'], '线下', $vv['member']['nickname'], $vv['create_time'], $vv['order_goods'][0]['original_price'], $vv['subtotal_price'], $vv['subtotal_coupon_price'], $vv['subtotal_share_platform_packet_price'], '已完成'];
            }
     ...
     * @throws \think\exception\DbException
     */
    public function confirmCollect(Request $request, OrderAttach $orderAttach, OrderGoods $orderGoods, MessageModel $message)
    {
        $args = $request::post();
        ...
        $orderAttachData = $orderAttach
            ...
            ->join('member m', 'm.member_id = oa.member_id')
            ->join('store s','s.store_id = oa.store_id')
            ->field('oa.order_attach_id,oa.status,oa.pay_type,oa.store_id,oa.subtotal_price,
            oa.express_value,oa.express_number,oa.sale_after_status,oa.after_sale_times,oa.member_id,
            m.web_open_id,m.subscribe_time,m.micro_open_id,m.phone,s.store_name,m.nickname')
            ->find();
        $args['member_id'] = $orderAttachData['member_id'];

        // 更改(店铺店铺和订单商品)订单状态
        if (!$orderAttachData || !$orderAttachData['order_goods_confirm_collect'] || !in_array($orderAttachData['status'], [2])) {
            return [
                'code' => -1,
                'message' => '订单商品不存在',
            ];
        ...
        $orderAttachData->pay_time = date('Y-m-d H:i:s');
        // 如果货到付款  支付方式为货到付款
        ...
        if ($refundArr) {
            OrderGoodsRefundModel::destroy(implode(',', $refundArr));
        }
        // 货到付款订单记录店铺资金记录
        if ($orderAttachData->pay_type == 2) {
            $storeCapitalData = [
        ...
        $pushServer->send([
            'tplKey' => 'order_state',
            'openId' => $orderAttachData['web_open_id'],
            'subscribe_time' => $orderAttachData['subscribe_time'],
            'microId' => $orderAttachData['micro_open_id'],
            'phone' => $orderAttachData['phone'],
            'data' => [6, $orderAttachData['store_name'], $orderAttachData['nickname']],
            'inside_data' => [
    ...
    public function offline_payment_info(Request $request, Cache $cache, MemberModel $member, MemberRank $memberRank, MemberGrowthRecord $memberGrowthRecord)
    {
        if ($request::isPost()) {
            try {
                $code = $request::param();
                ...
                if (empty($user_info)) {
                    return ['code' => '-100', 'message' => '用户不存在,请核对后重新输入'];
                }
                $growth_value = $memberGrowthRecord->where('member_id', $user_info['member_id'])->value('growth_value');
    ...
    //获得付款信息
    public function get_offline_payment_info(MemberModel $member, Request $request, MemberCoupon $memberCoupon,MemberPacket $memberPacket)
    {
        if ($request::isPost()) {
            try {
                $client_store_id = config('user.one_store_id');
                $data = $request::param();
                ...
                $discount_price = 0;//round($price - $data['price'], 2);
                if($price == 0){
                    $pay_price = $coupon =  $member_packet_id = $packet = $pay_price = $member_coupon_id = 0;
                }else{
                //保存优惠券查询对象
    ...
    //线下订单支付
    public function offline_payment_pay(Request $request, MemberPacket $memberPacket, MemberCoupon $memberCoupon, OrderModel $order, OrderAttach $orderAttach, OrderGoods $orderGoods, StoreCapital $storeCapital, Consumption $consumption, MemberModel $member, IntegralRecord $integralRecord, MemberGrowthRecord $growthRecord)
    {
        if ($request::isPost()) {
            //积分
            Env::load(Env::get('APP_PATH') . 'common/ini/.config');
