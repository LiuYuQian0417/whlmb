...
class Invoice extends Controller
{
    ...
    public function settings(Request $request)
    {
        ...
        if ($request::isPost()) {
            ...
            try {
                $param = $request::post();
                $param = [$param['switchType'] => $param['checked']];
                ...
                Db::commit();
                return ['code' => 0, 'message' => config('message.')[0], 'url' => '/invoice/settings'];
            } catch (\Exception $e) {
        ...
        if ($request::isPost()) {
            ...
            try {
                ...
                if (!empty($param['download_links'])) {
                    if (!filter_var($param['download_links'],FILTER_VALIDATE_URL)){
                        return ['code' => -100, 'message' => '发票下载链接格式错误'];
                    }
                }

                $param['billing_type'] = 1; // 正票
                $param['manage_nickname'] = Session::get('manageName');
                $param['bill_time'] = date('Y-m-d H:i:s');

                $invoice->isUpdate(TRUE)->allowField(TRUE)->save($param);

                Db::commit();
                return ['code' => 0, 'message' => config('message.')[0], 'url' => '/invoice/index'];
    ...
    public function RCW(Request $request, ExpressModel $express, InvoiceModel $invoice)
    {
        if ($request::isPost()) {
            ...
            try {
                ...
                Db::commit();
                return ['code' => 0, 'message' => config('message.')[0], 'url' => '/invoice/open'];
            } catch (\Exception $e) {
                return ['code' => 0, 'message' => config('message.')[-1]];
            }
        }
    ...
    public function getOrder($order_attach_id)
    {
        ...
        $goods = (new OrderGoodsModel())
            ...
            ->join('Goods Goods','order_goods.goods_id = Goods.goods_id','left')
            ->field('order_goods.*,Goods.file as file,(single_price*quantity) as goodsTotal,goods_evaluate_id,goods_evaluate.create_time as goods_evaluate_create_time')
            ->select();
