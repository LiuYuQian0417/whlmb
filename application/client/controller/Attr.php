...
//        }
    ...
    public function create(Request $request, AttrTypeModel $attrType, AttrModel $attr)
    {
        if ($request::isPost()) {
            try {
                ...
                if ($state) return ['code' => 0, 'message' => config('message.')[0], 'url' => '/client/attr/index?attr_type_id=' . $param['attr_type_id']];
            } catch (\Exception $e) {
                return ['code' => -100, 'message' => $e->getMessage()];
            }
        }
        return $this->fetch('', [
            'attrTypeData' => $attrType->where(['attr_type_id' => $request::get('attr_type_id')])->field('attr_type_id,type_name')->find()
        ]);
    }
    ...
    public function edit(Request $request, AttrTypeModel $attrType, AttrModel $attr)
    {
        ...
        return $this->fetch('create', [
            'item' => $data,
            'attrTypeData' => $attrType->where(['attr_type_id' => $data['attr_type_id']])->field('attr_type_id,type_name')->find()
        ]);
    }
