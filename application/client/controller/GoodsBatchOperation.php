...
//        try {
            ...
            if($request::isPost()){
                ...
                if($param['type']==1){
                    ...
                    $data = $goods->where('goods_sn|goods_id','in',$param['id'])->update($where);
                }
                if ($data) return ['code' => 0, 'message' => config('message.')[0], 'url' => '/client/goods_batch_operation/index'];
            }else{
                if($param['status']==1){
                    //根据商品ID
                    $w ='goods_id';
                ...
                $data = Db::name('goods')->where($w,'in',$param['id']) ->where('store_id',Session::get('client_store_id'))->field('goods_sn,brand_id,market_price,shop_price,goods_number,warn_number,goods_id,goods_name')->select();
